...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileContent, cursorLine, cursorColumn, suggestionType, fileName, aiSettings } =
      body;
    ...
    const suggestion = await generateSuggestion(prompt, aiSettings);
...
async function generateSuggestion(
  prompt: string,
  aiSettings?: CodeSuggestionRequest["aiSettings"]
): Promise<string> {
      ...
      response = await fetch(fetchUrl, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert code completion assistant. Output ONLY the code that should be inserted at the cursor. Do not include markdown code block formatting (```), explanations, or headers."
            },
            {
              role: "user",
              content: prompt
